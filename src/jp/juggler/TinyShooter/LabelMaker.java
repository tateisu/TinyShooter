...
public class LabelMaker {
    ...
    float mTexelWidth;  // Convert texel to U
    float mTexelHeight; // Convert texel to V
    int mU;
    int mV;
    int mLineHeight;
    ArrayList<Label> mLabels = new ArrayList<Label>();
    ...
    static class Label {
        ...
        Label(float width, float height, float baseLine,int cropU, int cropV, int cropW, int cropH) {
            this.width = width;
            this.height = height;
    ...
    public LabelMaker(boolean fullColor, int strikeWidth, int strikeHeight) {
    	strikeWidth =roundUpPower2(strikeWidth);
    	strikeHeight =roundUpPower2(strikeHeight);
     ...
     */
    public int add(GL10 gl, String text, Paint textPaint,int padd) {
        return add(gl, null, text, textPaint,padd);
    }
     ...
     * @return the id of the label, used to measure and draw the label
     */
    public int add(GL10 gl, Drawable background, String text, Paint textPaint,int padd) {
        return add(gl, background, text, textPaint,padd , 0, 0);
    }
     ...
     * @return the id of the label, used to measure and draw the label
     */
    public int add(GL10 gl, Drawable drawable,int padd, int minWidth, int minHeight) {
        return add(gl, drawable, null, null,padd , minWidth, minHeight);
    }
    ...
    public int add(
        ...
        if (v + lineHeight > mStrikeHeight) {
            throw new IllegalArgumentException(String.format("Out of texture space. label=%s,height %d,%d"
            		,text
            		,v + lineHeight
            		,mStrikeHeight
            ));
